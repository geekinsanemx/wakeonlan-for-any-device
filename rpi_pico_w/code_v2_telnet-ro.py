...
TELNET_HOST = "0.0.0.0"  # Listen on all interfaces
TELNET_PORT = 23         # Telnet default port
...
async def handle_telnet_client(client_sock):
    """
    Handles Telnet communication asynchronously in CircuitPython.
    """
    try:
        print(f"Client connected: {client_sock}")
        client_sock.send(b"Welcome to the Pico W UART bridge!\r\n")
...
async def start_telnet_server():
    pool = socketpool.SocketPool(wifi.radio)
    print("Starting Telnet server on port 23...")
    server_sock = pool.socket(pool.AF_INET, pool.SOCK_STREAM)
    server_sock.bind(("0.0.0.0", 23))
    server_sock.listen(5)
    server_sock.setblocking(False)  # Non-blocking socket

    while True:
        try:
            client_sock, addr = server_sock.accept()
            print(f"Connection from {addr}")
            asyncio.create_task(handle_telnet_client(client_sock))
        except Exception:
...
if __name__ == "__main__":
    while True:
        if manage_wifi_connection():  # Initial Wi-Fi connection
            ...
            loop.create_task(start_telnet_server())
            loop.run_forever()
        else:
